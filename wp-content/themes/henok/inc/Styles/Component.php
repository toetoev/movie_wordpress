<?php
/**
 * Henok\Henok\Styles\Component class
 *
 * @package Henok
 */

namespace Henok\Henok\Styles;

use Henok\Henok\Component_Interface;
use Henok\Henok\Templating_Component_Interface;
use function Henok\Henok\henok;
...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function action_enqueue_styles() {
		...
		if ( ! empty( $google_fonts_url ) ) {
			wp_enqueue_style( 'henok-fonts', $google_fonts_url, array(), null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
		...
		foreach ( $css_files as $handle => $data ) {
			...
			$version = henok()->get_asset_version( $css_dir . $data['file'] );
	...
	public function filter_resource_hints( array $urls, string $relation_type ) : array {
		if ( 'preconnect' === $relation_type && wp_style_is( 'henok-fonts', 'queue' ) ) {
			$urls[] = array(
		...
		$handles   = array_filter(
			...
			function( $handle ) use ( $css_files ) {
				...
				if ( ! $is_valid ) {
					...
					_doing_it_wrong( __CLASS__ . '::print_styles()', esc_html( sprintf( __( 'Invalid theme stylesheet handle: %s', 'henok' ), $handle ) ), 'Henok 2.0.0' );
				}
				return $is_valid;
			}
		);
	 ...
	 * {@see 'henok_preloading_styles_enabled'} filter can be used to tweak the return value.
	 *
	...
	protected function preloading_styles_enabled() {
		$preloading_styles_enabled = ! henok()->is_amp();
		 ...
		 */
		return apply_filters( 'henok_preloading_styles_enabled', $preloading_styles_enabled );
	}
	...
	protected function get_css_files() : array {
		...
		$css_files = array(
			...
			'henok-sidebar'    => array(
				...
				'preload_callback' => function() {
					return henok()->is_primary_sidebar_active();
				},
			),
			'henok-widgets'        => array(
			...
			'henok-sportspress' => array(
				'file'             => 'sportspress.min.css',
				/* To do -only preload if the plugin is active. */
				'preload_callback' => '__return_true',
			),
		);
		 ...
		 */
		$css_files = apply_filters( 'henok_css_files', $css_files );
	...
	protected function get_google_fonts() : array {
		 ...
		 */
		$this->google_fonts = (array) apply_filters( 'henok_google_fonts', $google_fonts );
