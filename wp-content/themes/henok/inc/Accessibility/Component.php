<?php
/**
 * Henok\Henok\Accessibility\Component class
 *
 * @package Henok
 */

namespace Henok\Henok\Accessibility;
...
class Component implements Component_Interface {
	...
	public function action_enqueue_navigation_script() {
		...
		wp_enqueue_script(
			...
			[],
			henok()->get_asset_version( get_theme_file_path( '/assets/js/navigation.min.js' ) ),
			false
		);
		wp_script_add_data( 'henok-navigation', 'async', true );
		wp_script_add_data( 'henok-navigation', 'precache', true );
		wp_localize_script(
			'henok-navigation',
			'henokScreenReaderText',
			[
				'expand'   => __( 'Expand child menu', 'henok' ),
				'collapse' => __( 'Collapse child menu', 'henok' ),
			]
		);
	}
